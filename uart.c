...
/* Transmit a single byte on UART0, blocking until the transmit holding
 * register is empty (THRE bit set in U0LSR). */
void uart0_tx(u8 ch)
{
...
/* Initialise UART1 for the given baud rate. Used here for basic polled
 * transmit; the HC-05 receive path is interrupt-driven and configured
 * separately in bluetooth.c (bluetooth_init() re-applies the same pin and
 * baud-rate setup before enabling the RX interrupt). */
...
/* Transmit a single byte on UART1, blocking until the transmit holding
 * register is empty. */
void uart1_tx(u8 ch)
{
    while (!(U1LSR & 0x20));
